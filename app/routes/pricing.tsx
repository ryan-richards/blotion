...
export const meta: MetaFunction = ({ data }) => {
  return {
    ...
    description:
      "Pricing for Blotion. Start your blog in minutes and use the power of Notion as a CMS.",
    "og:type": "website",
    ...
    "twitter:description":
      "Blotion pricing page. Start your blog in minutes and use the power of Notion as a CMS.",
  };
};
...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  if (userData.plan === "free") {
    ...
    return json(
      { status: userData.plan, ipAddress },
      {
        headers: {
          "Cache-Control": "s-maxage=3600, stale-while-revalidate=86400",
        },
...
export default function Pricing() {
  ...
  const redirectURL = canManage
    ? `/api/create-customer-portal-session`
    : `/api/create-checkout-session?plan=pro_${frequency}`;
  ...
  return (
    <Flex justify={"center"} align={"center"}>
      <Box py={{ base: "34px", md: "54px" }} px="10">
        <Flex
          justify={"flex-start"}
          mb={3}
          display={status != "not-logged-in" ? "flex" : "none"}
        >
            ...
            >
              ...
              >
                ...
                <Form
                  action={
                    canPurchase
                      ? `/api/create-checkout-session?plan=creative_${frequency}`
                      : "/api/create-customer-portal-session"
                  }
                  method="post"
                >
