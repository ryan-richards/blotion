...
export default function Settings() {
  ...
  let [customDomain, setCustomDomain] = useState(
    page.custom_domain ? page.custom_domain : ""
  );
  ...
  let customDomainStatus = customDomainAction.data
    ? customDomainAction.data.status
    : "";
  ...
  const handleNameCheck = async (value: any) => {
    ...
    if (regTest(value) && value != "") {
      ...
      setInputError(
        "Subdomain name invalid lowercase a-z 0-9 only! (no spaces)"
      );
    }
  };
  ...
  useEffect(() => {
    if (siteNameAvailable.type === "done" && siteNameAvailable.data.nameFree) {
      ...
      siteNameAvailable.type === "done" &&
      !siteNameAvailable.data.nameFree
    ) {
      setNameCheckCount(nameCheckCount + 1);
      if (input != subdomain) {
        setSiteNameValid(false);
  ...
  const customDomainConfigured = checkCustomDomain.data
    ? checkCustomDomain.data.valid
      ? true
      : false
    : false;
  ...
  const addCustomDomain = async (value: any) => {
    ...
    customDomainAction.submit(formData, {
      method: "post",
      action: "/api/domain",
    });
  };
    ...
    >
          ...
          <Stack>
            ...
            >
              <Link href={`https://${page.site_name}.blotion.com`} isExternal>
                <Text key={page.id}>
                  https://{input.toLowerCase()}.blotion.com
                </Text>
              </Link>
            </Flex>
          </Stack>
      ...
      <Flex mt={5} direction={"column"}>
        <Form method="post" autoComplete="false">
          ...
          <InputGroup gap={2}>
            <Input
              value={input}
              name={"site_name"}
              isInvalid={nameCheckCount > 0}
              errorBorderColor={!siteNameValid ? "red.500" : "green.400"}
              onChange={(e: any) => setInput(e.target.value)}
              pattern={"[0-9a-z_.-]*"}
              onBlur={(e: any) => handleNameCheck(e.target.value)}
            />
            <Button
              type={"submit"}
              isDisabled={!siteNameValid}
        ...
        <Text
          ...
          paddingTop={1}
          paddingBottom={1}
          display={!siteNameValid && nameCheckCount > 0 ? "flex" : "none"}
        >
          {inputError === ""
            ? "Site name already taken, try a different name."
            : null}
        </Text>
        <Text
          fontSize={"sm"}
      ...
      <Flex mt={5} direction={"column"}>
        ...
        <InputGroup gap={2}>
          <Input
            ...
            name={"custom_domain"}
            value={customDomain}
            isDisabled={userData.plan === "free" || !page.published}
          ...
          {!page.custom_domain ? (
            ...
            <Button
              ...
              colorScheme={"red"}
              isDisabled={!customDomain}
              onClick={() => removeCustomDomain(customDomain)}
              isLoading={isSubmitting}
            >
              Remove
            </Button>
        ...
        <Flex
          ...
          align={"center"}
          mt={3}
          display={customDomainConfigured ? "flex" : "none"}
        >
          <Text color={"green.400"}>Domain is successfully configured</Text>
          <Text color={"green.400"}>
            Please allow 48hr for DNS changes to take effect
          </Text>
        </Flex>
      </Flex>
        ...
        <Flex direction={"column"} mt={5}>
          <TableContainer>
            <Table variant="simple">
              <TableCaption>
                Set the above record on your DNS provider to continue
              </TableCaption>
              <Thead>
                <Tr>
                  <Th>Type</Th>
            ...
            <InputGroup gap={2}>
              ...
              <Form method={"post"}>
                <Button
                  ...
                  name={"action"}
                  value={"delete_site"}
                  isDisabled={deleteInput != deleteConfirm}
                  colorScheme={"red"}
                >
                  Delete
                </Button>
