...
export default function Settings() {
    ...
    const handleEnable = async () => {
        ...
        enableMetrics.submit(
            formData,
            { method: "post", action: "/api/metrics/add-website" }
        );
    }
    ...
    return (
        <Box bg={'box'} width={'full'} mt={10} p={{ base: 2, md: 10 }} rounded={'lg'}>
            ...
            {page.umami_website_id || enableMetrics.data ?
                null :
                <>
                    <Divider mt={5}></Divider>
                    <Flex mt={5} direction={'column'} gap={2} display={page.umami_website_id || enableMetrics.data ? 'none' : 'flex'}>
                        <Flex justify={'center'}>
                            <Button colorScheme={'blue'} onClick={handleEnable} isLoading={isSubmitting}>Enable Site Metrics</Button>
                        </Flex>
                    </Flex>
                    <Divider mt={5}></Divider>
                </>
            ...
            <Flex display={page.umami_website_id || enableMetrics.data ? 'flex' : 'none'} direction={'column'} align={'center'} gap={4} mt={4}>
                ...
                <Text>Temporary password is <b>'admin'</b></Text>
            </Flex>
        </Box >
    )
}