...
const keyHash = crypto.createHash('sha512').update(key, 'utf-8').digest('hex').substr(0, 32);
const ivHash = crypto.createHash('sha512').update(iv, 'utf-8').digest('hex').substr(0, 16);
...
export const decryptAPIKey = async (encryptedAPIKey: string) => {
    ...
    let decrypted = decipher.update(encryptedAPIKey, 'hex', 'utf8') + decipher.final('utf8');
    return decrypted;
}