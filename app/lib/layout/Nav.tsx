    ...
    <Flex direction={"row"} justify={"space-between"} mt={{ base: 2, md: 10 }}>
      <Link as={RemixLink} to={"/"}>
        <Heading size={"lg"} fontWeight={"semibold"}>
          {data.siteName ? data.siteName : data.site_name}
        </Heading>
      </Link>
        ...
        {navItems.length > 3 || isMobileMode ? (
            ...
            <MenuList zIndex={10}>
              {navItems.map((val: any) => (
                <Link
                  ...
                  key={val.slug}
                >
                  <MenuItem w="100%">{val.title}</MenuItem>
                </Link>
              ))}
            </MenuList>
          </Menu>
          ...
          <>
            {navItems.map((item: any) => (
              <Link key={item.slug} as={RemixLink} to={slugify(item.title)}>
                {item.title}
              </Link>
            ))}
          </>
        )}
        {data.users.plan === "free" ? null : <ThemeToggle />}
      </HStack>
    </Flex>
  );
}
